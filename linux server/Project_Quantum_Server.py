import socket, threading, sys, os, random, time, collidePlayer, struct, ast
...
class Player:
    def __init__(self, username, team, pos, vel, width, height):
        ...
        self.imgId = None
        self.powerup = 0 # 0 is standard, 1 is flower power
    ...
    def pack(self):
        return(";".join([":".join([str(self.pos[0])[:5], str(self.pos[1])[:5]]), str(self.lives), str(self.team), str(self.powerup), str(self.actionState), str(self.isAlive), str(self.username)]))
...
class Game:
    ...
    def pack(self):
        packedPlayers = "|".join([x.pack() for x in players])
        return(",".join([packedPlayers, "|".join(list(map(str, self.ps))), str(self.isActive), str(self.team0flags), str(self.team1flags)]))
...
class PlayerInput:
    ...
    def unpack(self, bin):
        ...
        self.left = ast.literal_eval(values[0])
        self.right = ast.literal_eval(values[1])
        self.up = ast.literal_eval(values[2])
        self.down = ast.literal_eval(values[3])

dataLock = threading.Lock()

#allows the host to enter console commands to control the server's operation
def hostInput():
    global connections, lobbies, tickInterval
...
def gameManager(connsId, gameId, playerId, mode, lobbyId):
    ...
    vRunMax = 1          # maximum speed when running
    aRun = 0.01           # acceleration when running
    vSprintCoeff = 1.5        # acceleration of run when sprinting as a multiple of vRunMax
    jumpCoeff = 1.3        # for jumping
    jumpSprintCoeff = 1   # increase in height when jumping while sprinting
    frictionCoeff = 3       # acceleration of friction when not going left/right as a multiple of aRun
    vFallMax = 1.5         # maximum speed when falling
    aFall = 0.015          # acceleration when falling
    ...
    try:
        ...
        if not dat:
            ...
            client.sendall(",".join([str(games[gameId].arena), str(playerId), str(len(games[gameId].ps))]).encode())
            dat = client.recv(1024)

            while True:
                startTime = time.time()

                client.sendall(games[gameId].pack().encode())
                ...
                if not dat:
                    ...
                    with dataLock:
                        ...
                        playerInputs[playerId].unpack(dat) # data now contains player inputs. physics engine is below.
                        ...
                        if playerInputs[playerId].down:
                            doneDown = True
                        else: doneDown = False

                        # left/right acceleration
                        if time.time() - disableRunAccTimer >= 0.1:
                            if playerInputs[playerId].left and players[playerId].vel[0] - (aRun*sprintCoeff) > -1*vRunMax*sprintCoeff:
                                players[playerId].vel[0] -= sprintCoeff*aRun
                            if playerInputs[playerId].right and players[playerId].vel[0] + (aRun*sprintCoeff) < vRunMax*sprintCoeff:
                                players[playerId].vel[0] += sprintCoeff*aRun

                        # falling acceleration, else friction
                        if not players[playerId].grounded:
                            players[playerId].vel[1] += aFall
                        else:
                            if players[playerId].vel[0] > 0 and not playerInputs[playerId].right:
                                if players[playerId].vel[0] - aRun*frictionCoeff >= 0: players[playerId].vel[0] -= aRun*frictionCoeff
                                else: players[playerId].vel[0] = 0
                            elif players[playerId].vel[0] < 0 and not playerInputs[playerId].left:
                                if players[playerId].vel[0] + aRun*frictionCoeff <= 0: players[playerId].vel[0] += aRun*frictionCoeff
                                else: players[playerId].vel[0] = 0
                        ...
                        players[playerId].pos[0] += players[playerId].vel[0]
                        players[playerId].pos[1] += players[playerId].vel[1]
